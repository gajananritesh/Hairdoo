...
#
#    if value.model_name == 'account':
#        pattern = "%s%s/%s" % ('customadmin/','users'+ '/'+ '1','update/')
#
#    elif value.model_name == 'eventinvitation' :
#        pattern = "%s:%s-%s" % ('core', 'event', arg)
#    else:
#        pattern = "%s:%s-%s" % ('core', value.model_name, arg)
#
#    return pattern
...
class MyUserUpdateView(
    ...
    def get_success_url(self):
        ...
        opts = self.model._meta
        return reverse('core:user-update',kwargs={"pk": self.get_object().pk})
...
class MyRegistredUserUpdateView(
    LoginRequiredMixin,
    PermissionRequiredMixin,
    UpdateView,
):
    ...
    def get_success_url(self):
        print("MyUpdateView::get_success_url")
        opts = self.model._meta
        return reverse('core:registred-users')
